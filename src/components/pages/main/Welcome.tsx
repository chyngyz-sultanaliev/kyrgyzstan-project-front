        ...
        <div className="absolute right-10 top-1/2 -translate-y-1/2 flex flex-row space-x-6 overflow-x-auto w-[840px] h-[400px] py-4 scrollbar-none scroll-width-[none]">
          {categories.map((c, i) => (
            <motion.div
              ...
              className="group relative min-w-[240px] h-[300px] rounded-2xl overflow-hidden shadow-xl bg-white/10 backdrop-blur-xl border border-white/20 hover:scale-105 transition-transform duration-300"
            >
              <img
                src={c.img}
                className="w-full h-full object-cover group-hover:scale-110 duration-500"
              />
              <div className="absolute bottom-0 w-full bg-gradient-to-t from-black/70 to-transparent p-4 text-center text-lg font-semibold tracking-wide">
                {c.title}
              </div>
            </motion.div>
          ))}
