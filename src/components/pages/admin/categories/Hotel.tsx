...
} from "@/shared/api/hotelCategoryApi";
import {
...
interface HotelCategoryFormData {
  name: string;
  image: string;
}

const HotelCategoryAdmin = () => {
  const { data: categories, isLoading, error } = useGetHotelCategoriesQuery();
  const [createCategory, { isLoading: isCreating }] =
    useCreateHotelCategoryMutation();
  const [updateCategory, { isLoading: isUpdating }] =
    useUpdateHotelCategoryMutation();
  const [deleteCategory] = useDeleteHotelCategoryMutation();

  const [editingId, setEditingId] = useState<string | null>(null);
  const [imagePreview, setImagePreview] = useState<string | null>(null);

  const { data: editingCategory } = useGetHotelCategoryByIdQuery(editingId!, {
    skip: !editingId,
  });
  ...
  const onSubmit = async (data: HotelCategoryFormData) => {
    try {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
              ...
              <div className="md:col-span-2">
                ...
                <div
                  ...
                  onClick={() =>
                    document
                      .getElementById("hotel-category-image-upload")
                      ?.click()
                  }
                ...
                >
                  <input
                    type="file"
                    id="hotel-category-image-upload"
                    className="hidden"
                    accept="image/*"
...
export default HotelCategoryAdmin;
