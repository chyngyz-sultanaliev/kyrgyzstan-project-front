...
// src/components/pages/admin/carCategory/CarCategoryAdmin.tsx
"use client";
...
} from "@/shared/api/carCategoryApi";
...
const CarCategoryAdmin = () => {
  const { data: categories, isLoading, error } = useGetCarCategoriesQuery();
  const [createCategory, { isLoading: isCreating }] =
    useCreateCarCategoryMutation();
  const [updateCategory, { isLoading: isUpdating }] =
    useUpdateCarCategoryMutation();
  const [deleteCategory] = useDeleteCarCategoryMutation();

  const [editingId, setEditingId] = useState<string | null>(null);
  const [imagePreview, setImagePreview] = useState<string | null>(null);

  const { data: editingCategory } = useGetCarCategoryByIdQuery(editingId!, {
    skip: !editingId,
  });
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<CarCategoryFormData>({
    defaultValues: { name: "", seats: 0, withDriver: false, images: "" },
  });

  useEffect(() => {
    if (editingCategory) {
      setValue("name", editingCategory.name);
      setValue("seats", editingCategory.seats);
      setValue("withDriver", editingCategory.withDriver);
      setValue("images", editingCategory.images || "");
      setImagePreview(editingCategory.images || null);
    }
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onloadend = () => {
      ...
      setValue("images", result, { shouldValidate: true });
    };
  ...
  const onSubmit = async (data: CarCategoryFormData) => {
    try {
      if (!data.name || !data.seats)
        return toast.error("Заполните обязательные поля");
      if (!data.images) return toast.error("Добавьте изображение");
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="p-4 sm:p-6 lg:p-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
              <div className="md:col-span-2">
                ...
                <input
                  type="text"
                  placeholder="Например: Эконом, Бизнес-класс"
              ...
              <div className="flex items-center">
                <div className="flex items-center gap-3 bg-gray-50 px-4 py-3 rounded-lg w-full">
                  <input
                    type="checkbox"
                    {...register("withDriver")}
                    id="withDriver"
                    className="w-5 h-5 text-[#0A8791] rounded focus:ring-[#0A8791]"
                  />
                  <label
                    htmlFor="withDriver"
              ...
              <div className="md:col-span-2">
                ...
                <div
                  className={`border-2 border-dashed rounded-lg p-6 sm:p-8 text-center cursor-pointer transition-all hover:border-[#0A8791] hover:bg-gray-50 ${
                ...
                >
                  ...
                  />
                  <Upload className="w-8 h-8 sm:w-12 sm:h-12 mx-auto mb-3 text-gray-400" />
        ...
        <div>
          ...
          {!categories || categories.length === 0 ? (
            <div className="bg-white rounded-xl shadow-md p-8 sm:p-12 text-center">
              ...
              <p className="text-gray-500 text-lg">Пока нет категорий</p>
              <p className="text-gray-400 text-sm mt-2">
                Создайте первую категорию выше
              </p>
            </div>
          ) : (
                  ...
                  <div className="p-4">
                    ...
                    <div className="space-y-1 mb-4">
                      ...
                      <p className="text-sm text-gray-600 flex items-center gap-2">
                        ...
                        <span>
                          {cat.withDriver ? "С водителем" : "Без водителя"}
                        </span>
                      </p>
                    </div>
...
export default CarCategoryAdmin;
